...
import { transferDepartmentJob } from '../../api/trasnfer-department-job';
import { Button } from '../../components/button';
import { notify } from '../../components/notification';
import { Job } from '../../types/job';

interface TransferJobToDepartmentProps {
  changeEditJobModal: () => void;
  job: Job;
  isOpen: boolean;
}

export default function TransferJobToDepartment({
  job,
  changeEditJobModal,
  isOpen,
}: TransferJobToDepartmentProps) {
  ...
  const { mutateAsync: transferDepartmentJobFn, isPending } = useMutation({
    mutationFn: transferDepartmentJob,
  });

  async function handleTransferDepartment() {
    try {
      transferDepartmentJobFn({
      ...
      navigate('/departamento/requisicoes');
    ...
    <div className="flex flex-col gap-2">
      ...
      <form action="submit" className="flex flex-col gap-2">
        ...
        <Button
          ...
          disabled={isPending || disabledButtonTransferDepartment}
        >
          {isPending ? (
            <>
              <svg
